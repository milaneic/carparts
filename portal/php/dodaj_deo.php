        ...
        <div class="container">
            ...
            <form method="POST" enctype="multipart/form-data" action='#'>
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" id="naziv" name='naziv' placeholder="Unesi naziv">
                </div>
                <div class="form-group">
                    <label for="proizvodjac">Proizvodjac</label>

                    <input type="text" class="form-control" id="proizvodjac" name='proizvodjac' placeholder="Unesi proizvodjaca">
                </div>
                <div class="form-group">
                    <label for="opis">Opis proizvoda</label>
                    <input type="text" class="form-control" id="opis" name='opis' placeholder="Unesi opis proizvoda">
                </div>
                <div class="form-group">
                    <label for="cena">Cena proizvoda bez PDV-a:</label>
                    <input type="text" class="form-control" id="cena" name='cena' onblur="PDV(this)" placeholder="Unesi cenu proizvoda">
                </div>
                <div class="form-group">
                    <label for="cenaPDV">Cena proizvoda sa PDV-om:</label>
                    <input type="text" class="form-control" id="cenaPDV" readonly name='cenaPDV' placeholder="Unesi cenu proizvoda sa PDV-om">
                </div>
                <div class="form-group">
                    <label for="slika">Slika proizvoda</label><br>

                    <input type="file" id="slika" name="fileToUpload">
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <select name="marka" id="marka" class="form-control">
                        ...
                        foreach ($marke as $k) {
                            echo $k['marka'];
                        ?>
                            <option value="<?= $k['id']  ?>"><?= $k['marka'] ?></option>
                        <?php  }  ?>
                    </select>
                </div>
...
<script>
    function PDV(obj) {
        var cena = parseFloat(obj.value);
        var cPDV = cena * 1.2;
        document.getElementById('cenaPDV').value = cPDV;
    }
</script>
...
if (isset($_POST["submit"])) {
    ...
    if ($naziv != '' && $proizvodjac != '' && $opis != '' && $kategorija != '' && $marka != '' && $_FILES['fileToUpload']['name'] != "") {
        echo "usao";
        $sql = "INSERT INTO delovi VALUES('null','" . $proizvodjac . "','" . $naziv . "','" . $opis . "','/portal/img/products" . $_FILES["fileToUpload"]["name"] . "','" . $cena . "','" . $cenaPDV . "','" . $kategorija . "','" . $marka . "')";
        if ($db->query($sql) === TRUE) {
            echo '<script>
        window.location.href="../admin.php";
        </script>';
        } else {
            echo "Error: " . $sql . " in " . $db->error;
        }
    } else {
        echo '<script>
        alert("Sva polja nisu popunjena!");
        window.location.href="dodaj_deo.php";
        </script>';
    }
}
?>